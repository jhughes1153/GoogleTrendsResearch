...
class GoogleTrends:
    ...
    def scrape_data(self, keywords):
        ...
        def add_blank_column(keyword):
            ...
            nums = np.zeros(len(self.keywords.date_values))
        ...
        driver = webdriver.Firefox(executable_path=r'C:\Users\Jack\Downloads\geckodriver-v0.22.0-win64\geckodriver.exe', options = options)
            ...
            if keyword not in self.keywords.columns:
                ...
                while(breakout < 3):
                    try:
                        ...
                        if(self.state == 0):
                            ...
                            if(self.date_range_week):
                                ...
                                for i in range(len(dates)):
                                    ...
                                    dates_conv.append(my_date + dt.timedelta(days=1))
                                self.keywords['date_values'] = dates_conv
                                self.state = 1
                            else:
                                for i in range(len(dates)):
                        ...
                        print(self.keywords.shape)
                        print(len(nums))
                        #try this just in case it is a different size in the file
                        if(len(nums) == len(self.keywords.date_values)):
                            self.keywords[keyword] = nums
...
if __name__ == '__main__':
    ...
    googleTrendsScrape = GoogleTrends('aapl', ['Apple'])
