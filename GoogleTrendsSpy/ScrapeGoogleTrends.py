...
class GoogleTrends:
            ...
            while breakout < 3:
                try:
                    ...
                    if breakout < 3:
                        ...
                        self.keywords_df[keyword.replace(' ', '_').replace('&', 'and')] = np.zeros(
                            len(self.keywords_df.date_values))
    ...
    def keywords_to_csv(self):
        ...
        if os.name == 'posix':
            ...
            self.keywords_df.to_csv('C:\\Users' + username + '\\googlekeywords\\\\' + self.quote + 'matrix.csv',
                                    index=False)
        else:
